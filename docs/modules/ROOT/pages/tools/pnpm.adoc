...
== Workspace script routing

For monorepos: where `dev` / `build` script names come from, `--filter` vs `-C`, and running app scripts from the **repo root** without `cd`, see the General Knowledge component: xref:general-knowledge::reference/pnpm-run-routing.adoc[pnpm run routing].
