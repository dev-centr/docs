...
* **Host UX:** link:https://github.com/dev-centr/easy-installer[Ibex / easy-installer] — `installer.kdl`, plugin ABI, in-place PATH, file-manager menus (`ibex` CLI).
* **Packaging:** link:https://github.com/dev-centr/msi-generator[msi-generator] — JSON/CLI spec, multi-file MSIX with assets, and a real OLE2 CFB + uncompressed CAB + core MSI tables (v0.2). Ibex hosts the project UX and plugins on top.
