...
Layout, fonts, and default teal chrome stay valentus-theme. `devcentr-brand.css` remaps `--adt-accent*` tokens to the org palette and sets `--adt-nav-item-toggle*` to the same accent so component-level nav expand/collapse chevrons read as branded (not muted ink). TIP/NOTE colors stay on Default UI / Valentus admonition tokens (`--tip-color` / `--adt-admonition-tip` — Antora green by default; do not remap tip to brand accent):

* Light primary: `#178F80`
* Dark primary: `#2DD4BF`
* Logo accents: `#14B8A6` / `#0F766E`
* Nav-tree expand chevrons: `--adt-nav-item-toggle` / `--adt-nav-item-toggle-hover` → brand accent (Valentus **2.0.15+**)
