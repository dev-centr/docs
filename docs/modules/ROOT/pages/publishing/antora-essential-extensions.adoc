...
**Preferred:** one `nav:` entry on ROOT; nest modules with `include::module$nav.adoc[]` under the linked parent (`* xref:DevCentr:secrets-manager:index.adoc[Secrets Manager]`). The included file uses `**` / `***` depth under that parent.
