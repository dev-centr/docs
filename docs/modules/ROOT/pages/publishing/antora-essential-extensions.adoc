...
**Preferred:** one `nav:` entry on ROOT; nest modules with `include::module$nav.adoc[]` under the linked parent (`* xref:platforms:secrets-manager:index.adoc[Secrets Manager]`). The included file uses `**` / `***` depth under that parent.

[.themed-svg]
image::nav-typology.svg[Nav typology: single tree with includes vs parallel multi-nav trees,width=760]

Hub companion JS (`supplemental-ui/js/site-nav-tree-current.js`) prefers the deepest `is-current-page`, expands only that ancestry (`is-current-path`), and restores other open trees from `sessionStorage` across navigations. In-doc link crossfade + scroll preservation: `supplemental-ui/js/site-doc-nav.js`.
