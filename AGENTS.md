...
- Diagrams: Kroki bake for PlantUML (+ Themed SVG pipeline live/unmaintained); **client Mermaid** via `@antora-supplemental/mermaid-client` (CDN default). Lightbox (zoom/pan): `@antora-supplemental/diagram-lightbox`. See `publishing/antora-diagram-formats.adoc`. Compose pack: `antora-supplemental/antora-facto` — do not force extras into Valentus `v2`.
