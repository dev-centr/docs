...
module.exports.register = function () {
  this.on('contentClassified', ({ contentCatalog }) => {
    ...
    if (details.length > 0) {
      logger.warn(
        `${details.length} themed-SVG dark-mode text issue(s) -- see advice below (or antora-diagram-formats); fix agents scanning warn/error should treat this as actionable.`
      )
      for (const gap of details) {
        // Per-file detail at info (not WARN) so default Antora log level stays quiet.
        logger.info(gap)
      }
    }
  })
