...
| `supplemental-ui/` | Minimal site overlay on the valentus-theme UI bundle (custom domain `CNAME`; optional branding SVGs under `img/`). Header logo and navbar text use playbook `site.keys`—no `header-content.hbs` override needed (valentus-theme v1.1.0+).
