...
| `supplemental-ui/` | Minimal site overlay on the antora-dark-theme UI bundle (custom domain `CNAME`; optional branding SVGs under `img/`). Header logo and navbar text use playbook `site.keys`—no `header-content.hbs` override needed (antora-dark-theme v1.0.12+).
